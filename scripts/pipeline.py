...
def paraphrase(
	...
	*,
	...
	for bi in tqdm(range(0, len(idxs), max(1, batch_size)), desc="Paraphrase", unit="batch"):
		...
		# Encode and move to same device as model embeddings
		enc = tok(
			batch_texts,
			return_tensors="pt",
			truncation=True,
		...
		# Always use CUDA for single-GPU setup
		emb_device = torch.device("cuda")
		_validate_ids(enc, model, "Pegasus-Paraphrase in paraphrase")
		...
		dec = tok.batch_decode(out, skip_special_tokens=True) if out is not None else [""] * len(batch_ids)
		for j, idx in enumerate(batch_ids):
